...
export default function Navbar() {
  ...
  // Navbar is visible only at the very top of the page (over the Hero). As soon
  // as you scroll it auto-hides and only reveals on hover (or when the mobile
  // menu is open) -- on the Hero and every other section alike.
  ...
  const visible = !scrolled || hovered || menuOpen;

  return (
    <>
      {/* Thin hover strip at the very top: reveals the navbar once it has
          auto-hidden after you scroll. */}
      <div
        aria-hidden
        onMouseEnter={() => setHovered(true)}
        className="fixed inset-x-0 top-0 z-[999] h-5"
      />
      <header
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        className={`fixed inset-x-0 top-0 z-[1000] border-b transition-[transform,opacity,background,border-color,box-shadow] duration-[600ms] ease-out-expo will-change-transform ${
